...
enum PipelineStates {
    ...
    static func createParticleRenderPSO(
        pixelFormat: MTLPixelFormat,
        enableBlending: Bool = false
    ) -> MTLRenderPipelineState {
        let vertexFunction =
        Renderer.library.makeFunction(name: "vertex_particle")
        let fragmentFunction =
        Renderer.library.makeFunction(name: "fragment_particle")
    ...
    static func createComputePSO(function: String) -> MTLComputePipelineState {
        guard let kernel = Renderer.library.makeFunction(name: function) else {
